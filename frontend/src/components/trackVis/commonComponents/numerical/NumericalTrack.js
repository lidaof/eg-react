...
class NumericalTrack extends React.PureComponent {
  ...
  renderTooltip(relativeX) {
    ...
    const value2 = this.hasReverse
      ? this.xToValue2[Math.round(relativeX)]
      : null;
    const stringValue =
  ...
  render() {
    ...
    this.xToValue2 =
      smooth === 0
        ? xToValue2BeforeSmooth
        : Smooth(xToValue2BeforeSmooth, smooth);
    const xValues2 = this.xToValue2.filter(x => x);
    if (xValues2.length) {
    ...
    this.xToValue =
      smooth === 0
        ...
        : Smooth(xToValueBeforeSmooth, smooth);
    ...
    const legend = (
      <TrackLegend
        ...
        axisScaleReverse={
          isDrawingBars ? this.scales.valueToYReverse : undefined
        }
        axisLegend={unit}
      />
    );

...
export const GLViewer = (function () {
    ...
    function GLViewer(element, config) {
        ...
        var raycaster = new $3Dmol.Raycaster(new $3Dmol.Vector3(0, 0, 0), new $3Dmol.Vector3(0, 0, 0));
    ...
    $.get('data/4csv.pdb', function(data) {
      ...
      viewer.render(callback);
        ...
        this.zoomTo = function (sel, animationDuration, fixedPath) {
            ...
            maxD = Math.sqrt(maxDsq) * 2;
            var finalpos = center.clone().multiplyScalar(-1);
            var finalz = -((maxD * 0.5) / Math.tan(((Math.PI / 180.0) * camera.fov) / 2) - CAMERA_Z);

            finalz = adjustZoomToLimits(finalz);
            if (animationDuration > 0) {
                animateMotion(animationDuration, fixedPath, finalpos, finalz, rotationGroup.quaternion, lookingAt);
            } else {
                modelGroup.position = finalpos;
        ...
        var getSelectionCenter = function (spec) {
            if (spec.hasOwnProperty("x") && spec.hasOwnProperty("y") && spec.hasOwnProperty("z")) return spec;
        ...
        function addLineDashed(spec, s) {
            ...
            var p1 = new $3Dmol.Vector3(spec.start.x || 0, spec.start.y || 0, spec.start.z || 0);
            var p2 = new $3Dmol.Vector3(spec.end.x, spec.end.y || 0, spec.end.z || 0);
     ...
     * than addVolumetricData, but can not be used with py3Dmol.
       ...
       viewer.render();
         ...
         * @return {Object}
         */
        var generateMeshSyncHelper = function (type, expandedExtent, extendedAtoms, atomsToShow, atoms, vol) {
            //            var time = new Date();
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        var rfunction = function (event) {
                            ...
                            for (var i = 0, vl = VandFs.length; i < vl; i++) {
                                var VandF = { vertices: VandFs[i].vertexArr, faces: VandFs[i].faceArr };
                                var mesh = generateSurfaceMesh(atomlist, VandF, mat);
                                $3Dmol.mergeGeos(surfobj.geo, mesh);
                            }
                            _viewer.render();
        ...
        this.setAutoEyeSeparation = function (isright) {
            ...
            if (isright || camera.position.x > 0)
                //setting a value of dist*tan(5)
                camera.position.x = dist * Math.tan((Math.PI / 180.0) * 5.0);
            else camera.position.x = -dist * Math.tan((Math.PI / 180.0) * 5.0);

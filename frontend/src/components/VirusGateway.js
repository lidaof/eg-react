...
class VirusGateway extends React.Component {
    ...
    handleSubmit = async event => {
        ...
        const virus = new VirusGenome(this.state.name, virusGateway[this.state.name].fastaUrl);
        await virus.init();
        const genome = new Genome(this.state.name, [new Chromosome(virus._seqId, virus._length)]);
        ...
        this.props.onCustomVirusGenome(
            ...
            virus._seqId,
            virus._seq,
            virusGateway[genome._name].tracks.filter(track => !track.fileObj).map(track => track.serialize())
        );

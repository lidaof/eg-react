...
import { ActionCreators, AppState } from '../AppState';
...
class SessionUINotConnected extends React.Component<SessionUIProps, SessionUIState> {
	...
	renderSavedSessions = () => {
		...
		const buttons = sessions.map(([id, session]) => {
			...
			return (
				<li key={id}>
					<span style={{ marginRight: '1ch' }}>{session.label}</span>({new Date(session.date).toLocaleString()}){button}
					{deleteButton}
				</li>
			);
		});
		...
		return (
			<div className="SessionUI-sessionlist">
				Sort session by:{' '}
				<label>
					<input type="radio" value="date" name="sort" checked={sortSession === 'date'} onChange={this.onSortChange} />
					<span>Date</span>
				</label>
				<label>
					<input type="radio" name="sort" value="label" checked={sortSession === 'label'} onChange={this.onSortChange} />
					<span>Label</span>
				</label>
				<ol>{buttons}</ol>
			</div>
			...
			<div style={{ margin: '20px', display: this.props.withGenomePicker ? 'flex' : 'block', flexDirection: 'column', alignItems: 'center' }}>
				<div>
					<label htmlFor="retrieveId">
						<input type="text" size={50} placeholder="Session bundle Id" value={this.state.retrieveId} onChange={this.setRetrieveId} />
					</label>
					...
					<React.Fragment>
						<div>
							...
							<label htmlFor="sessionLabel">
								Name your session: <input type="text" value={this.state.newSessionLabel} size={40} onChange={this.setSessionLabel} /> or use a{' '}
								<button type="button" className="SessionUI btn btn-warning btn-sm" onClick={this.setRandomLabel}>
									{' '}
									Random name
								</button>
							</label>
				...
				<div className="font-italic" style={{ maxWidth: '480px' }}>
					Disclaimer: please use <span className="font-weight-bold">sessionFile</span> or <span className="font-weight-bold">hub</span> URL for publishing
					using the Browser. Session id is supposed to be shared with trusted people only. Please check our docs for{' '}
...
const enhance = compose<typeof SessionUINotConnected>(

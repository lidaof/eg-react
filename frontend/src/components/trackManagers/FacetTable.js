...
class FacetTable extends Component {
  ...
  initializeTracks(allTracks) {
    ...
    const rowHeader = metaKeys.includes(DEFAULT_ROW)
      ? DEFAULT_ROW
      : metaKeys[0];
    let columnHeader = metaKeys.includes(DEFAULT_COLUMN)
      ? DEFAULT_COLUMN
      : metaKeys[1];
    const rowList = [
      {
        ...
        if (row.expanded) {
          ...
          divs.push(
            <div key={`${row.name}-col`}>
              {this.countTracks(row, UNUSED_META_KEY)}
            </div>
          );
        }
      }
  ...
  countTracks(row, col) {
    ...
    for (let track of tracks) {
      ...
      if (
        row.name === rowHeader ||
        this.trackMetadataBelongsTo(tkRowInfo, row.name)
      ) {
        ...
        if (col === UNUSED_META_KEY) {
          ...
          if (
            ...
            this.trackMetadataBelongsTo(tkColumnInfo, col.name)
          ) {
            found.push(track);
          }
        }
    ...
    const addUrls = found.filter(
      tk =>
        this.props.addedTrackSets.has(tk.url) ||
        this.props.addedTrackSets.has(tk.name)
    );
    return (
      <div>

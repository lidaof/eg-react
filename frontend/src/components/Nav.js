...
class Nav extends React.Component {
  ...
  renderOtherGenomes() {
    ...
    const otherGenomes = allGenomes
      .map(g => g.genome.getName())
      .filter(g => g !== genomeName);
    const radios = otherGenomes.map(g => {
      const { name } = getSpeciesInfo(g);
      return (
        <label key={g}>
          <Radio value={g} /> <span className="capitalize">{name}</span>{" "}
  ...
  render() {
    ...
    const hasInteractionTrack = tracks.some(model =>
      INTERACTION_TYPES.includes(model.type)
    )
      ? true
      : false;
    const REGION_EXPANDER = hasInteractionTrack
      ? REGION_EXPANDER1
      : REGION_EXPANDER0;
        ...
        >
          ...
          >
            ...
            <button
              ...
              onClick={this.handleGenomeCloseModal}
            >
              Close
            </button>{" "}
            {otherGenome && (
  ...
  return (
    <React.Fragment>
      ...
      <ColorPicker
        color={color}
        onChange={onChange}
        label="current highlight box color"
        disableAlpha={false}
      />
    </React.Fragment>
  );

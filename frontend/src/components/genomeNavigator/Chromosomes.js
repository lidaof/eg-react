...
class Chromosomes extends React.PureComponent {
    ...
    async fetchSequence(props) {
        ...
        if (drawModel.basesToXWidth(1) > Sequence.MIN_X_WIDTH_PER_BASE) {
            if (this.props.fastaSeq) {
                const sequence = this.props.fastaSeq.slice(0, 10);
    ...
    render() {
        ...
        const cytobands = viewRegion.getGenomeIntervals().map(locus => this.renderCytobandsInLocus(locus, drawModel));

...
class Chromosomes extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.twoBitSource = props.genomeConfig.twoBitURL
      ? new TwoBitSource(props.genomeConfig.twoBitURL)
      : null;
    this.fastaSeq = props.genomeConfig.fastaSeq
      ? props.genomeConfig.fastaSeq
      : "";
    this.fetchSequence = _.throttle(this.fetchSequence, 500);
    // this.fetchSequence(props);

    this.featurePlacer = new FeaturePlacer();
    this.featurePlacer.placeFeatures = memoizeOne(
      this.featurePlacer.placeFeatures
    );
  }
  ...
  fetchSequence = async props => {
    ...
    if (drawModel.basesToXWidth(1) > Sequence.MIN_X_WIDTH_PER_BASE) {
      ...
      if (this.fastaSeq.length) {
        ...
        sequence = props.viewRegion
          .getGenomeIntervals()
          .map(locus => new SequenceData(locus, seq));
  ...
  renderOneCytoband(cytoband, cytobandLocus, drawModel) {
    ...
    for (const contextInterval of contextIntervals) {
      ...
      const endX = Math.min(
        drawModel.baseToX(contextInterval.end),
        drawModel.getDrawWidth()
      );
  ...
  renderCytobandsInLocus(locus, drawModel) {
    ...
    for (let cytoband of cytobandsForChr) {
      ...
      if (cytobandLocus.getOverlap(locus)) {
        children.push(
          this.renderOneCytoband(cytoband, cytobandLocus, drawModel)
        );
      }
    }
    return children;
  ...
  getSizeForFeatureLabel(label, maxWidth) {
    return FEATURE_LABEL_SIZES.find(
      size => label.length * size * 0.6 < maxWidth
    );
  }
  ...
  render() {
    ...
    return (
      <TranslatableG x={this.props.x} y={this.props.y}>
        {boxesAndLabels}
        {cytobands}
        {drawModel.basesToXWidth(1) > Sequence.MIN_X_WIDTH_PER_BASE &&

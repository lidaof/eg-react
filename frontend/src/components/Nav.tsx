...
class Nav extends React.Component {
	...
	renderOtherGenomes() {
		...
		return (
			<RadioGroup name="otherGenome" selectedValue={this.state.otherGenome} onChange={this.handleOtherGenomeChange} className="otherGenome-container">
				{radios}
			</RadioGroup>
		);
	}
						...
						<div className="dropdown-menu">
							...
							{!process.env.REACT_APP_NO_FIREBASE && (
								<>
									<ModalMenuItem
										itemLabel="Session"
										// @ts-ignore dunno why this is here, but it probably serves a purpose
										style={{
											content: {
												right: 'unset',
												bottom: 'unset',
							...
							<ModalMenuItem
								...
								<ScreenshotUI expansionAmount={REGION_EXPANDER} needClip={hasInteractionTrack} genomeConfig={genomeConfig} />
							</ModalMenuItem>
							<ModalMenuItem
					...
					<div className="element Nav-center">
						...
						<div className="dropdown-menu">
							<label className="dropdown-item" htmlFor="switchNavigator">
								<input id="switchNavigator" type="checkbox" checked={isShowingNavigator} onChange={onToggleNavigator} />
							...
							<label className="dropdown-item" htmlFor="isHighlightRegion">
								<input id="isHighlightRegion" type="checkbox" checked={highlightEnteredRegion} onChange={onToggleHighlight} />
					...
					{!virusBrowserMode && (
						<div className="element Nav-center">
							...
							<div className="dropdown-menu">
								...
								<label className="dropdown-item">
									<a href="https://epigenomegateway.readthedocs.io/" target="_blank" rel="noopener noreferrer">
										Documentation
									</a>
								</label>
								<label className="dropdown-item">
									<a href="http://epigenomegateway.wustl.edu/legacy" target="_blank" rel="noopener noreferrer">
										The 'old' browser
									</a>
								</label>
								<label className="dropdown-item">
									<a href="https://groups.google.com/d/forum/epgg" target="_blank" rel="noopener noreferrer">
										Google groups
									</a>
								</label>
								<label className="dropdown-item">
								...
								<label className="dropdown-item">
									<a href="https://www.youtube.com/channel/UCnGVWbxJv-DPDCAFDQ1oFQA" target="_blank" rel="noopener noreferrer">
										YouTube channel
									</a>
								</label>
							</div>
...
interface HighlightColorChangeProps {
	color: string;
	onChange: ColorChangeHandler;
}

const HighlightColorChange: React.FC<HighlightColorChangeProps> = ({ color, onChange }) => {
	return (
		<>
...
const DropdownOpener: React.FC<DropdownOpenerProps> = ({ extraClassName, label }) => {
	return (
		<button type="button" className={`btn dropdown-toggle ${extraClassName}`} data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
			{label}
		</button>
	);
};
...
interface ModalMenuItemProps {
	itemLabel: string;
	style?: Partial<ReactModal.Styles>;
}

const ModalMenuItem: React.FC<React.PropsWithChildren<ModalMenuItemProps>> = ({ itemLabel, style: propStyle, children }) => {
	const [isOpen, setIsOpen] = useState<boolean>(false);
	const style = useMemo<ReactModal.Styles>(() => ({ overlay: { backgroundColor: 'rgba(111,107,101, 0.7)', zIndex: 4 }, ...propStyle }), [propStyle]);
	...
	return (
		<React.Fragment>
			...
			<ReactModal isOpen={isOpen} ariaHideApp={false} onRequestClose={toggleOpen} shouldCloseOnOverlayClick={true} style={style}>

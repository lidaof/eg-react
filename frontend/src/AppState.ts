...
export const ActionCreators = {
  ...
  setCustomVirusGenome: (genomeConfig: GenomeConfig) => {
    return { type: ActionType.SET_CUSTOM_VIRUS_GENOME, genomeConfig };
  }
};
...
function getInitialState(): AppState {
  ...
  if (!_.isEmpty(query)) {
    ...
    if (query.position) {
      const interval = newState.viewRegion
...
function getNextState(prevState: AppState, action: AppAction): AppState {
  ...
  switch (action.type) {
    case ActionType.SET_GENOME: // Setting genome resets state.
      ...
      if (genomeConfig) {
        nextViewRegion = new DisplayedRegionModel(
    ...
    case ActionType.SET_CUSTOM_VIRUS_GENOME: // Setting virus genome.
      const virusViewRegion = new DisplayedRegionModel(
        action.genomeConfig.navContext
      );
      const virusTracks = action.genomeConfig.defaultTracks;
      return {
        ...initialState,
        genomeName: action.genomeConfig.genome.getName(),
...
async function asyncInitState() {
  ...
  if (!_.isEmpty(query)) {
    if (query.hub) {
      ...
      if (customTracksPool) {
        const tracks = customTracksPool.filter(
          (track: any) => track.showOnHubLoad
        );
        if (tracks.length > 0) {
          AppState.dispatch(
            ActionCreators.setTracksCustomTracksPool(tracks, customTracksPool)
          );
        } else {
          AppState.dispatch(
            ActionCreators.setCustomTracksPool(customTracksPool)
          );
        }
      }
    }

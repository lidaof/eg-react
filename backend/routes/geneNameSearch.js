      ...
      validate: {
        ...
        query: {
          ...
          getOnlyNames: Joi.bool()
            .description(
              "Whether to get only a list of matching names, or full records"
            )
            .default(false)
        }
      }
...
async function queryGenesWithName(request, h) {
  ...
  const regExpChar = request.query.isExact ? '$': '';
  const query = {
    name: {
      $regex: `^${encodeURIComponent(request.query.q)}${regExpChar}`,
  ...
  let findResult, findResultWithCollection, findResults = [];
  const collections = genomeConfig[request.params.genome];
  for (const collection of collections){
    try {
      findResult = await mongoUtils
        .executeFind(
          mongoClient,
          request.params.genome,
          collection.name,
          query
        )
        .limit(RECORDS_LIMIT)
        .toArray();
        findResultWithCollection = findResult.map((result) => {return  {...result, 'collection': collection.name} } );
        findResults = findResults.concat(findResultWithCollection);
    } catch (error) {
      console.error(error);
  ...
  if (request.query.getOnlyNames) {
    return _.uniq(findResults.map(record => record.name));
  } else {
    return findResults;
  }
}
